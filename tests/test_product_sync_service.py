...
def test_string_price_plain_digits():
    service = ProductSyncService(session=None)
    item = {"name": "StrDigits", "price": "12999"}
    normalized = service._normalize_product_dto(item, None)
    assert normalized["price"] == 12999.0


def test_string_price_with_space():
    service = ProductSyncService(session=None)
    item = {"name": "StrWithSpace", "price": "12 999"}
    normalized = service._normalize_product_dto(item, None)
    assert normalized["price"] == 12999.0


def test_string_price_with_currency_suffix():
    service = ProductSyncService(session=None)
    item = {"name": "StrWithCurr", "price": "12999 грн"}
    normalized = service._normalize_product_dto(item, None)
    assert normalized["price"] == 12999.0
    # parsed currency should be detected by parse_price_value
    assert normalized["currency"] == "грн"